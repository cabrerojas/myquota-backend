...
export class TransactionService extends BaseService<Transaction> {
    ...
    async fetchBankEmails(): Promise<void> {
                ...
                await Promise.all(
                    chunk.map(async (messageId, index) => {
                        ...
                        if (encodedMessage) {
                            ...
                            const { amount, currency, cardLastDigits, merchant, transactionDate } = this.extractTransactionDataFromHtml(content);
    ...
    private findHtmlOrPlainText(part: gmail_v1.Schema$MessagePart): string | null | undefined {
